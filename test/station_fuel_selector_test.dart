...
class _Repository implements StationDiscoveryRepository {
  ...
  Future<StationDiscoveryResult> loadCurrentCity({bool forceRefresh = false}) async =>
      const StationDiscoveryResult(city: 'Bebedouro', stations: []);
  @override
  Future<StationDiscoveryResult> loadCity(String city, {bool forceRefresh = false}) async =>
      StationDiscoveryResult(city: city, stations: const []);
}

void main() {
  testWidgets('exibe três opções amplas com ícone e seleção evidente', (tester) async {
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: StationFuelSelector(
      selected: FuelType.gasoline, onSelected: (_) {},
    ))));

    expect(find.byType(StationFuelOption), findsNWidgets(3));
    expect(find.byIcon(Icons.local_gas_station_outlined), findsNWidgets(3));
    expect(tester.getSize(find.byType(StationFuelOption).first).height, greaterThanOrEqualTo(64));
    expect(find.byKey(const ValueKey('fuel-selected-gasoline')), findsOneWidget);
  });

  testWidgets('arrasto da direita para esquerda avança para etanol', (tester) async {
    await tester.pumpWidget(ProviderScope(
      overrides: [stationDiscoveryRepositoryProvider.overrideWithValue(_Repository())],
      child: const MaterialApp(home: StationDiscoveryPage()),
    ));
    await tester.pumpAndSettle();

    expect(find.byKey(const ValueKey('fuel-selected-gasoline')), findsOneWidget);
    await tester.fling(find.byType(CustomScrollView), const Offset(-300, 0), 1000);
    await tester.pumpAndSettle();
